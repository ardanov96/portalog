...
import {
  ...
  Bell, CreditCard, Trash2, RefreshCw, ChevronDown, ChevronUp,
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {Object.entries(result.stats).map(([key, val]) => (
              <div key={key} className="bg-white/60 rounded-lg px-2.5 py-1.5">
                <p className="text-[10px] text-slate-500 uppercase tracking-wide">{key.replace(/([A-Z])/g, ' $1').trim()}</p>
...
export default function CronPage() {
  ...
  const runJob = async (jobId: string, endpoint: string) => {
    setJobs(prev => prev.map(j => j.id === jobId ? { ...j, running: true, lastRun: undefined } : j))

    try {
      const res  = await fetch(endpoint, {
        headers: {
          // Kirim CRON_SECRET kalau ada (untuk test dari dashboard)
          Authorization: `Bearer ${process.env.NEXT_PUBLIC_CRON_SECRET ?? 'dev-test'}`,
        },
      })
      const data: CronResult = await res.json()
      setJobs(prev => prev.map(j => j.id === jobId ? { ...j, running: false, lastRun: data } : j))
    } catch (e: any) {
      setJobs(prev => prev.map(j =>
        j.id === jobId ? {
          ...j, running: false,
          lastRun: {
            job: jobId, startedAt: new Date().toISOString(), finishedAt: new Date().toISOString(),
            durationMs: 0, success: false, stats: {}, errors: [e.message],
          },
        } : j
      ))
    }
    ...
    <div className="max-w-3xl space-y-5">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {job.lastRun && !job.running && (
                      <span className={cn(
                        ...
                        {job.lastRun.success ? <Check className="w-2.5 h-2.5" /> : <AlertTriangle className="w-2.5 h-2.5" />}
                  ...
                  <div className="flex items-center gap-3 text-[10px] text-slate-400">
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{job.scheduleLabel}</span>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5">
        ...
        <ol className="space-y-2 text-xs text-blue-700 leading-relaxed">
          <li><strong>1.</strong> Pastikan file <code className="bg-blue-100 px-1.5 py-0.5 rounded font-mono text-[11px]">vercel.json</code> sudah ada di root project (sudah dibuat)</li>
          <li><strong>2.</strong> Di Vercel Dashboard → Settings → Environment Variables → tambahkan <code className="bg-blue-100 px-1.5 py-0.5 rounded font-mono text-[11px]">CRON_SECRET</code> dengan nilai random string panjang</li>
          <li><strong>3.</strong> Deploy ulang project → Vercel akan otomatis membaca jadwal dari <code className="bg-blue-100 px-1.5 py-0.5 rounded font-mono text-[11px]">vercel.json</code></li>
